...
final class LoggerStoreTests: BaseTestCase {
    ...
    func testSizeLimit() throws {
        ...
        store.storeRequest(
            URLRequest(url: URL(string: "example.com/delete-this")!),
            response: nil,
            error: nil,
    ...
    func testRemoveAll() throws {
        ...
        store.storeMessage(
            ...
            level: .debug,
            message: "test",
            metadata: ["hey": .string("this is meta yo")],
            file: #file,
            function: #function,
            line: #line)
    ...
    private func makeStore(
        options: LoggerStore.Options = [.create, .synchronous],
        _ closure: (inout LoggerStore.Configuration) -> Void = { _ in })
        -> LoggerStore {
        var configuration = LoggerStore.Configuration()
        configuration.makeCurrentDate = { [unowned self] in date }
