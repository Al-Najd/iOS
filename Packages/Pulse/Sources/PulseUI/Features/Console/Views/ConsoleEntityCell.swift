...
private struct _ConsoleTaskCell: View {
    ...
    var body: some View {
        ...
        if #available(iOS 15, *) {
            ...
            .sheet(item: $shareItems, content: ShareView.init)
            #else
            .popover(isPresented: $isSharing, attachmentAnchor: .point(.leading), arrowEdge: .leading) {
                ShareNetworkTaskView(task: task)
            }
            #endif
        } else {
            cell
