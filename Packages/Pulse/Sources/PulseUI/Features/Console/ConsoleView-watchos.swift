    ...
    var body: some View {
        HStack {
            if environment.initialMode == .all {
                ...
                .background(
                    environment.bindingForNetworkMode.wrappedValue
                        ? Rectangle().foregroundColor(.blue).cornerRadius(8)
                        : nil)
            }
            ...
            .background(
                viewModel.isDefaultFilters(for: environment.mode)
                    ? nil
                    : Rectangle().foregroundColor(.blue).cornerRadius(8))
        }
        .font(.title3)
