...
extension Parsers {
    ...
    static let filterStatusCode = oneOf(
        ...
        listOf(statusCode).filter { !$0.isEmpty }.map { (0.7, $0) }).map { confidence, values in
    ...
    static let filterMethod = oneOf(
        ...
        listOf(httpMethod).filter { !$0.isEmpty }.map { (0.7, $0) }).map { confidence, values in
    ...
    static func makeFilterPath(paths: Set<String>) -> Parser<[(ConsoleSearchFilter, Confidence)]> {
        oneOf(
            ...
            (char(from: "/") *> optional(path)).map { (0.7, ["/" + ($0 ?? "")]) }).map { confidence, values in
    ...
    static let rangeModifier: Parser<ConsoleSearchRangeModfier> = whitespaces *> oneOf(
        ...
        oneOf("<", ".<", "..<").map { _ in .open },
        string("..").map { _ in .closed }) <* whitespaces
}
