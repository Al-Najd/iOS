...
final class ConsoleSearchRecentSearchesStore {
    ...
    init(mode: ConsoleMode) {
        ...
        searches = decode([ConsoleSearchTerm].self, from: UserDefaults.standard.string(forKey: searchesKey) ?? "[]") ?? []
        filters = decode([ConsoleSearchFilter].self, from: UserDefaults.standard.string(forKey: filtersKey) ?? "[]") ?? []
    }
