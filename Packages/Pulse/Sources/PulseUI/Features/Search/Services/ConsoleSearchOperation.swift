...
final class ConsoleSearchOperation {
    ...
    init(
        ...
        hasLogFilters = parameters.filters.contains { $0.filter is (any ConsoleSearchLogFilterProtocol) }
        hasNetworkFilers = parameters.filters.contains { $0.filter is (any ConsoleSearchNetworkFilterProtocol) }
    ...
    private func _start() {
        ...
        while index < objectIDs.count, !isCancelled, !hasMore {
            ...
            if
                let entity = try? context.existingObject(with: objectIDs[index]),
                ...
                if found > cutoff {
                    ...
                    DispatchQueue.main.async {
                        self.delegate?.searchOperation(
                            self,
                            didAddResults: [ConsoleSearchResultViewModel(
                                entity: self.entities[currentMatchIndex],
    ...
    private static func search(
        ...
        _ scope: ConsoleSearchScope) -> [ConsoleSearchOccurrence] {
