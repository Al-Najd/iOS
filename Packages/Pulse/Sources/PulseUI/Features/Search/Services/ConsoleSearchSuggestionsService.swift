...
final class ConsoleNetworkSearchSuggestionsService {
    ...
    init(mode: ConsoleMode) {
        self.mode = mode
        recents = ConsoleSearchRecentSearchesStore(mode: mode)
    }
    ...
    private func parse(
        _ filters: [Parser<[(ConsoleSearchFilter, Confidence)]>],
        _ context: ConsoleSearchSuggestionsContext) -> [ConsoleSearchSuggestion] {
    ...
    private func makeSuggestion(for term: ConsoleSearchTerm) -> ConsoleSearchSuggestion {
        ConsoleSearchSuggestion(
            text: AttributedString("\(term.options.title) ") { $0.foregroundColor = .primary } +
