...
final class ConsoleSearchViewModel: ObservableObject, ConsoleSearchOperationDelegate {
    ...
    private func configure(mode: ConsoleMode) {
        suggestionsService = ConsoleNetworkSearchSuggestionsService(mode: mode)
        suggestionsViewModel = suggestionsService.makeSuggestions(for: makeContextForSuggestions())
        DispatchQueue.main.async {
            self.refreshNow()
        }
    }
    ...
    private func autocompleteCurrentFilter() {
        if
            let filter = suggestionsViewModel.filters.first,
    ...
    private func startSearch(parameters: ConsoleSearchParameters) {
        ...
        let operation = ConsoleSearchOperation(
    ...
    private func checkForNewSearchMatches(for entities: [NSManagedObject]) {
        ...
        operation.delegate = self
        operation.resume()
        refreshResultsOperation = operation
    }
    ...
    func searchOperation(_ operation: ConsoleSearchOperation, didAddResults results: [ConsoleSearchResultViewModel]) {
        if operation === self.operation {
        ...
        } else if operation === refreshResultsOperation {
            ...
            refreshResultsOperation?.cancel()
            refreshResultsOperation = nil
        }
    }
    ...
    func onSubmitSearch() {
        #if os(macOS)
        if
            let suggestionID = UUID(uuidString: searchBar.text),
...
struct ConsoleSearchParameters: Equatable, Hashable {
    ...
    init(tokens: [ConsoleSearchToken], scopes: Set<ConsoleSearchScope>) {
        for token in tokens {
            ...
            case .term(let string): terms.append(string)
            }
        }
