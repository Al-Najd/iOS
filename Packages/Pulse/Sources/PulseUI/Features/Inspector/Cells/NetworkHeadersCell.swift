...
// MARK: - NetworkHeadersCellViewModel
...
struct NetworkHeadersCellViewModel {
    ...
    init(title: String, headers: [String: String]?) {
        self.title = title
        let headers = headers ?? [:]
        details = "\(headers.count)"
        isEnabled = !headers.isEmpty
        ...
        NavigationView {
            List {
                ForEach(MockTask.allEntities, id: \.objectID) { task in
                    Section {
                        Text(task.url ?? "–")
                        NetworkHeadersCell(viewModel: .init(
                            title: "Original Request Headers",
                            headers: task.originalRequest?.headers))
                        NetworkHeadersCell(viewModel: .init(
                            title: "Current Request Headers",
                            headers: task.currentRequest?.headers))
