...
private func makeTimingRows(
    transaction: NetworkTransactionMetricsEntity,
    taskInterval: DateInterval) -> [TimingRowSectionViewModel] {
    var sections = [TimingRowSectionViewModel]()
    ...
    case .localCache:
        if
            let requestStartDate = timing.requestStartDate,
    ...
    case .networkLoad:
        ...
        if
            let fetchStartDate = timing.fetchStartDate, let endDate = earliestEventDate,
        ...
        if let domainLookupStartDate = timing.domainLookupStartDate {
            connection
                .append(makeRow(title: "DNS", color: .systemPurple, from: domainLookupStartDate, to: timing.domainLookupEndDate))
        }
        ...
        if let secureConnectionStartDate = timing.secureConnectionStartDate {
            connection
                .append(makeRow(
                    ...
                    to: timing.secureConnectionEndDate))
        }

        // Response Section

        let latestPreviousEndDate = timing.requestEndDate ?? timing.connectEndDate ?? timing.domainLookupEndDate ?? timing
            .fetchStartDate
