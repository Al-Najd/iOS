...
extension Parser: ExpressibleByStringLiteral, ExpressibleByUnicodeScalarLiteral,
    ExpressibleByExtendedGraphemeClusterLiteral where A == Void {
...
struct Confidence: Hashable, Comparable, ExpressibleByFloatLiteral {
    ...
    init(floatLiteral value: FloatLiteralType) {
        rawValue = max(0, min(1, Float(value)))
    }
...
extension String {
    ...
    func fuzzyMatch(_ target: String) -> Confidence {
        guard !isEmpty, !target.isEmpty else {
            return 0.0
        }
        let lhs = lowercased()
        ...
        let fullConfidence = 1 - (Float(fullDistance) / Float(max(count, target.count)))

        return Confidence((prefixConfidence * 3 + fullConfidence) / 4.0)
    }

    func distance(to rhs: String) -> Int {
        guard !rhs.isEmpty else { return count }
        guard !isEmpty else { return rhs.count }
