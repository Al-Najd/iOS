...
final class TextRenderer {
    ...
    func render(_ message: LoggerMessageEntity) {
        string.append(
            ConsoleFormatter.subheadline(for: message) + "\n",
    ...
    func render(_ task: NetworkTaskEntity, content: NetworkContent) {
        ...
        if
            content.contains(.originalRequestHeaders), content.contains(.currentRequestHeaders),
    ...
    func renderCompact(_ task: NetworkTaskEntity) {
        ...
        string.append(
            "\(time)\(ConsoleFormatter.separator)\(status)\(ConsoleFormatter.separator)",
            helper.attributes(role: .body2, style: .monospacedDigital, width: .condensed, color: titleColor))
        var urlAttributes = helper.attributes(role: .body2, weight: .medium, color: detailsColor)
    ...
    func render(
        _ blob: LoggerBlobHandleEntity,
        _ data: Data,
        contentType: NetworkLogger.ContentType?,
        error: NetworkLogger.DecodingError?)
        -> NSAttributedString {
    ...
    func render(
        _ section: KeyValueSectionViewModel,
        details: String? = nil,
        style: TextFontStyle = .monospaced)
        -> NSAttributedString {
    ...
    func render(_ values: [(String, String?)]?, color: Color, style: TextFontStyle = .monospaced) -> NSAttributedString {
        ...
        @discardableResult
        func append(_ value: String) -> NSRange {
...
struct ConsoleTextRenderer_Previews: PreviewProvider {
    static var previews: some View {
        Group {
            ...
            RichTextView(viewModel: .init(string: NSAttributedString(string: ShareStoreTask(
                entities: try! LoggerStore.mock
                    .allMessages(),
                store: .mock,
                output: .plainText,
                completion: { _ in }).share().items[0] as! String)))
                .previewDisplayName("Task (Plain)")

            RichTextView(viewModel: .init(
                string: TextRenderer(options: .sharing)
                    .make { $0.render(task.orderedTransactions[0]) }))
...
struct NetworkContent: OptionSet {
    ...
    static let summary: NetworkContent = [
        largeHeader, taskDetails, errorDetails, requestComponents, requestQueryItems, errorDetails, originalRequestHeaders,
        currentRequestHeaders, requestOptions, responseHeaders
    ]

    static let all: NetworkContent = [
        largeHeader, taskDetails, errorDetails, requestComponents, requestQueryItems, errorDetails, originalRequestHeaders,
        currentRequestHeaders, requestOptions, requestBody, responseHeaders, responseBody
    ]
}
