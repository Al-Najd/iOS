...
final class PinButtonViewModel: ObservableObject {
    ...
    init(_ message: LoggerMessageEntity) {
        self.message = message
        pins = message.managedObjectContext?.userInfo[pinServiceKey] as? LoggerStore.Pins
        subscribe()
    }

    init(_ task: NetworkTaskEntity) {
        message = task.message
        pins = task.managedObjectContext?.userInfo[pinServiceKey] as? LoggerStore.Pins
        subscribe()
    }
