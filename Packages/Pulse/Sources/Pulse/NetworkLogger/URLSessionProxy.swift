...
// MARK: Experimental.URLSessionProxy
    ...
    final class URLSessionProxy {
        ...
        private init() {
            session = URLSession(
                ...
                delegate: URLSessionProxyDelegate(logger: logger, delegate: CustomProtocolSessionDelegate.shared),
                delegateQueue: CustomProtocolSessionDelegate.shared.queue)
        }
...
// MARK: - CustomProtocolSessionDelegate
...
private final class CustomProtocolSessionDelegate: NSObject, URLSessionTaskDelegate, URLSessionDataDelegate {
    ...
    func urlSession(
        ...
        didReceive response: URLResponse,
        completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        handlers[dataTask]?.proto
            .urlSession(session, dataTask: dataTask, didReceive: response, completionHandler: completionHandler)
    }
    ...
    func urlSession(
        ...
        newRequest request: URLRequest,
        completionHandler: @escaping (URLRequest?) -> Void) {
        handlers[task]?.proto.urlSession(
            session,
            task: task,
