...
public final class URLSessionProxyDelegate: NSObject, URLSessionTaskDelegate, URLSessionDataDelegate, URLSessionDownloadDelegate {
    ...
    public func urlSession(
        ...
        (actualDelegate as? URLSessionTaskDelegate)?.urlSession?(
            session,
            task: task,
    ...
    public func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        (actualDelegate as? URLSessionDownloadDelegate)?
    ...
    public func urlSession(
        ...
        (actualDelegate as? URLSessionDownloadDelegate)?.urlSession?(
            session,
...
public extension URLSessionProxyDelegate {
    ...
    static func enableAutomaticRegistration(logger: NetworkLogger = .init()) {
        sharedLogger = logger
        if
            let lhs = class_getClassMethod(URLSession.self, #selector(URLSession.init(configuration:delegate:delegateQueue:))),
            let rhs = class_getClassMethod(
                URLSession.self,
                #selector(URLSession.pulse_init(configuration:delegate:delegateQueue:))) {
