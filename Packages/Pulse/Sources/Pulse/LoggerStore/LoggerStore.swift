...
public final class LoggerStore: @unchecked Sendable {
    ...
    public init(storeURL: URL, options: Options = [], configuration: Configuration = .init()) throws {
        ...
        isArchive = fileExists && !isDirectory.boolValue
        ...
        if !isArchive {
            ...
            guard try Version(string: info.storeVersion) >= .minimumSupportedVersion else {
                throw LoggerStore.Error.unsupportedVersion(
                    ...
                    minimumSupportedVersion: Version.minimumSupportedVersion.description)
            }
            // Extract and decompress _only_ the database. The blobs can be read
            // directly from the compressed archive on demand.
            databaseURL = URL.temp.appending(filename: info.storeId.uuidString)
            if !Files.fileExists(atPath: databaseURL.path) {
                try document.database().decompressed().write(to: databaseURL)
            }
            self.document = .archive(info, document)
            manifest = .init(storeId: info.storeId, version: try Version(string: info.storeVersion))
        }

        container = LoggerStore.makeContainer(databaseURL: databaseURL)
...
extension LoggerStore {
    ...
    private func process(_ event: Event.NetworkTaskCompleted) {
        ...
        entity.isFromCache = transactions.last?
            .fetchType == .localCache ||
            (transactions.last?.fetchType == .networkLoad && transactions.last?.response?.statusCode == 304)
    ...
    private func preprocessData(_ data: Data, contentType: NetworkLogger.ContentType?) -> Data {
        ...
        guard
            let thumbnail = Graphics.makeThumbnail(from: data, targetSize: 512),
    ...
    private func findOrCreateTask(
        forTaskId taskId: UUID,
        taskType: NetworkLogger.TaskType,
        createdAt: Date,
        label: String?,
        url: URL?)
        -> NetworkTaskEntity {
    ...
    private func makeTransaction(
        at index: Int,
        transaction: NetworkLogger.TransactionMetrics)
    ...
    private func storeBlob(_ data: Data, contentType: NetworkLogger.ContentType?) -> LoggerBlobHandleEntity? {
        ...
        if
            configuration.isBlobCompressionEnabled, let compressed = try? data.compressed(),
            compressed.count < Int(Double(data.count) * 1.1) {
...
extension LoggerStore {
    ...
    public func export(
        to targetURL: URL,
        as docType: DocumentType = .archive,
        options: ExportOptions = .init()) async throws
        -> Info {
        ...
        return try document.context.performAndReturn {
            ...
            if Files.fileExists(atPath: blobsURL.path) {
                ...
                for chunk in blobURLs.chunked(into: 100) {
                    ...
                    try document.context
                        .execute(NSBatchInsertRequest(entityName: String(describing: PulseBlobEntity.self), objects: objects))
                }
            }
    ...
    enum Error: Swift.Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .unsupportedVersion(
                let version,
                let minimumSupportedVersion): return "The store was created by with Pulse vesrion \(version). Minimum supported version is \(minimumSupportedVersion)."
