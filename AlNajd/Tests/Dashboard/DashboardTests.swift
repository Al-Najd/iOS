        ...
        describe("the dashboard") {
            describe("ability to generate analysis") {
                context("on comparing two different ranges") {
                    let emptyWeekSeeder: (inout [Date: [Deed]], Date) -> () = { dictionary, date in
                        dictionary[date] = .faraaid
                    }
                    
                    let maxedOutWeekSeeder: (inout [Date: [Deed]], Date) -> () = { dictionary, date in
                        dictionary[date] = .faraaid.map { $0.changing { $0.isDone = true }}
                    }
                    
                    let goodWeekSeeder: (inout [Date: [Deed]], Date) -> () = { dictionary, date in
                        var deeds: [Deed] = .faraaid
                        (0...3).forEach { deeds[$0].isDone = true }
                        dictionary[date] = deeds
                    }
                    
                    let badWeekSeeder: (inout [Date: [Deed]], Date) -> () = { dictionary, date in
                        var deeds: [Deed] = .faraaid
